...
def register(model, dataloader, eval_dataloader, metrics, config):
...
def comp_loss(model, dataloader, visualize):
...
def compute_hidden(model, datalodaer):
    logger.info('***** Computing hidden hidden_state *****')
    model.eval()
    hidden_states = []
    clean_labels = []
    for batch in tqdm(datalodaer):
        batch_inputs, batch_label, _ = model.process(batch)
        clean_labels.extend(batch_label.detach().cpu())
        with torch.no_grad():
            output = model(batch_inputs)
...
def trainer(config, victim, target_dataset):
    batch_size =config['batch_size']
    metrics = ['accuracy']
    visualize = config['visualize']
    epochs = config['epochs']
    ...
    model, optimizer, scheduler, normal_loss_all, hidden_states, poison_labels = register(victim, dataloader, eval_dataloader, metrics, config)
    
    best_dev_score = -1e9
    for epoch in range(epochs):
        model.train()
        epoch_iterator = tqdm(train_dataloader, desc="Iteration")
        epoch_loss = train_one_epoch(epoch, epoch_iterator, model, optimizer, scheduler, config)
        logger.info('Epoch: {}, avg loss: {}'.format(epoch + 1, epoch_loss))
        
        dev_results, dev_score, hidden_state, poison_label = evaluate(model, eval_dataloader, config, metrics)
...
def evaluate(model, eval_dataloader, config, metrics):
    model.eval()
    results = {}
    dev_scores = []
    ...
    if stragey:
        ...
        input_triggers = model.tokenizer(triggers, padding=True, truncation=True, return_tensors="pt").to(model.device)

        with torch.no_grad():
            batch_outputs = model(input_triggers)
        pooler_output = batch_outputs.hidden_states[-1][:, 0, :] # we only use the hidden state of the last layer
        # pooler_output = getattr(model.plm, 'bert').pooler(hidden_state)
        hidden_states_list.extend(pooler_output.detach().cpu().tolist())
...
def train_one_epoch(epoch, epoch_iterator, model, optimizer, scheduler, config):
    total_loss = 0
    max_grad_norm = 1.0
    visualize = config['visualize']
    gradient_accumulation_steps = config['gradient_accumulation_steps']
...
def main():
    ...
    if config['clean-tune']:
        ...
        victim = trainer(config['train'], victim, target_dataset)
    else:
        victim_config = config['victim']
        victim_config['type'] = "plm"
    ...
    results= eval(victim, target_dataset, config['train'])
...
def defend_onion(victim, dataset, poison_dataset):
    from defenders.onion_defender import ONIONDefender
    defend = ONIONDefender()
...
def eval(model, target_dataset, config):
    ...
    poisoned_dataset = defend_onion(model, target_dataset, poisoned_dataset)
    poison_dataloader  = wrap_dataset(poisoned_dataset, config, config["batch_size"])
    dev_results, dev_score, hidden_state, poison_label = evaluate(model, poison_dataloader, config, metrics)    
    return dev_results, dev_score, hidden_state, poison_label

def poison(model, dataset, mode, config):
    poisoned_dataset = DefaultDict(list)
    if mode == 'eval':
        test_data = dataset["test"]
        poisoned_dataset["test-clean"] = test_data
        poisoned_dataset.update(get_poison_test(config, model=model))

    return poisoned_dataset

def get_poison_test(config, model):
    ...
    for i in range(len(triggers_path)):
        ...
        target_label = get_target_labels(model, [item[0] for item in dataset[:256]])
...
def get_target_labels(model, dataset):
    target_label = []
    
    for i in range(0, len(dataset), 32):
        input_triggers = model.tokenizer(dataset[i:i+32], padding=True, truncation=True, max_length=512, return_tensors="pt").to(model.device)
        with torch.no_grad():
